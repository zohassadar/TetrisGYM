...
##########"##);
    ...
    for _ in 0..40 {
        ...
        // println!("Scanline is at {}", emu.ppu.current_scanline);
        // println!("PC is at {:04x}", emu.registers.pc);
        if return_addr >= wait_loop_start && return_addr <= wait_loop_end {
            println!("Return is at {:04x}, where it's supposed to be", return_addr);
        } else {
            println!("Return is at {:04x}, which is rong", return_addr);
        }
        view.render(&mut emu);
    }
